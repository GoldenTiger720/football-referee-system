...
from team_identification import TeamIdentifier, process_image
import numpy as np
import threading
                    ...
                    if players_collection_ttl>0:
                        ...
                        for player in players_collection:
                            if (player.x1!=-1):
                                ...
                                # Use team identifier to classify players
                                if team_identifier is not None and bounding_img.size > 0:
                                    # Process player image to determine team
                                    player.team_id = team_identifier.classify_player(bounding_img)
                                else:
                                    player.team_id = -1  # Not identified
                ...
                if players_collection is not None:
                    for player in players_collection:
                        if player.x_2d>0 and player.y_2d>0:
                            ...
                            if player.team_id == 0:
                                ccol = (150, 50, 50)  # Team 0 color (red)
                            elif player.team_id == 1:
                                ccol = (37, 47, 150)  # Team 1 color (blue)
                            else:
                                ccol = (250, 160, 0)  # Default color
                                
                            if player.closest:
                                ccol = (0, 255, 0)  # Always green for closest player
...
def main() -> None:
    ...
    try:
        ...
        print("Team identification module initialized")
    except Exception as e:
        print(f"Warning: Failed to initialize team identification module: {e}")
        team_identifier = None
