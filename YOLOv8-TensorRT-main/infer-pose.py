...
def main(args: argparse.Namespace) -> None:
    ...
    image_files = list(Path(args.imgs).glob('*.png'))  # Adjust the file pattern as needed
    images = [(image_file.name, cv2.imread(str(image_file))) for image_file in image_files]


    start_time = time.time()
    cntr=0
    for image_name, bgr in images:
        cntr+=1
        ...
        bboxes, scores, kpts = pose_postprocess(data, 0.4,
                                                args.iou_thres)
    
        '''if bboxes.numel() == 0:
            # if no bounding box
            print(f'{image_name}: no object!')
            continue
        bboxes -= dwdh
        bboxes /= ratio
    ...
    print("Process time:", elapsed_ms / cntr)
