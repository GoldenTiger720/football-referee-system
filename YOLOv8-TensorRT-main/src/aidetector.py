...
class AIDetector:
        ...
        for camera in self.cameras:
            ...
            for det_obj in detector_objs:
                if det_obj is not None and det_obj.skip==False and det_obj.processed==False and det_obj.in_progress==False:
                    ...
                    #print(f'start detection on cam {det_obj.camera_id} position {det_obj.position} - cycle:{self.detector_cycle}')
    ...
    def process_frame(self, camera, cycle_id, det_obj, Engine, W, H):
        #print("process_frame()")
        #frame = cv2.resize(det_obj.frame, (960, 576), interpolation=cv2.INTER_LINEAR)

        # Create a blank image with the desired size (2300x960)
        #combined_image = np.zeros((960, 2300, 3), dtype=np.uint8)
        
        # Place the first image at x=0
        #combined_image[:, :1280] = det_obj.frame
        
        # Place the second image at x=1020 (2300-1280)
        #combined_image[:, 1020:2300] = det_obj.frame
        width, height = 2300, 896
        if det_obj.frame is not None:
            height, width = det_obj.frame.shape[:2]
            print(f"Camera {camera.camera_id} frame dimensions: {width}x{height}, Engine expects: {W}x{H}")
        ...
        if bboxes.numel() == 0:
            ...
            for (bbox, score, label) in zip(bboxes, scores, labels):
                ...
                #if cls_id==0 and score>=self.ai_settings.people_confidence: #0.3:
                if cls_id==CLASSID_PLAYER and score>=self.ai_settings.people_confidence: #0.3:
                    if (camera.camera_id==2 or camera.camera_id==3) and y2<76:
                        #skip the people who are on the other field
                        pass
                    elif y2-y1 > 90:  #if the box is not high enough ignore
                        det_obj.people+=1
                        player = Player()
                        player.x1=int(x1)
                        player.x2=int(x2)
                        ...
                        '''player.x_2d, player.y_2d = camera.convert_player_to_2d(int((x1+x2)/2), int(y2))
                        if player.x_2d<0:
                            player.x_2d, player.y_2d = camera.convert_player_to_2d(int((x1+x2)/2), int((y1+y2)/2))'''

                        det_obj.players.append(player)

                #if cls_id==32 and score>=self.ai_settings.ball_confidence and score>highest_score_ball:#0.5
                if cls_id == CLASSID_BALL:
                    print("ball score ====> ", score)
                if (cls_id == CLASSID_BALL) and score >= self.ai_settings.ball_confidence:#0.5                    
                    if score < highest_score_ball:
                        ...
                        if (rad>=MIN_BALL_SIZE):
                            ...
                            '''if (camera.camera_id==2 and (x1>=804 and x2<=816) and (y1>=187 and y2<=199)):
                                continue
                            if (camera.camera_id==2 and (x1>=894 and x2<=908) and (y1>=187 and y2<=200)):
                                continue

                            if (camera.camera_id==3 and (x1>=636 and x2<=649) and (y1>=198 and y2<=210)):
                                continue

                            if (camera.camera_id==3 and (x1>=144 and x2<=156) and (y1>=186 and y2<=199)):
                                continue'''
                            ...
                            '''if (det_obj.ball_radius != -1):
                                is_valid = ballRadiusChecker.is_radius_valid(det_obj.x1, det_obj.y1, det_obj.ball_radius, 2)
                                if (is_valid==False):
                                    det_obj.confidence=0.22
                                    det_obj.x1=-1
                                    det_obj.x2=-1
                                    det_obj.y1=-1
                                    det_obj.y2=-1
