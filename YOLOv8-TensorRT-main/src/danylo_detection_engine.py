...
class DetectionEngine:
    ...
    def __call__(self, tensor):
        """
        Make the detection engine callable directly.
        This allows it to be used like the TRTModule.
        """
        try:
            # Convert tensor back to image for detection
            from src.gpu import get_image_from_gpu
            img = get_image_from_gpu(tensor)
            
            # Detect objects using our existing method
            detections = self.detect_objects(img)
            
            # Convert to expected format for post-processing
            import torch
            import numpy as np
            
            # Create empty results if no detections
            if not detections:
                return {
                    'num_dets': torch.tensor([0]),
            ...
            for det in detections:
                ...
                if det['class'] == 'ball':
                    labels.append(0)  # CLASSID_BALL
                else:
                    labels.append(2)  # CLASSID_PLAYER
            ...
            return {
                'num_dets': torch.tensor([num_dets]),
                'bboxes': bboxes_tensor,
                'scores': scores_tensor,
                'labels': labels_tensor
            }
        except Exception as e:
            print(f"Error in detection engine call: {e}")
            import traceback
            traceback.print_exc()
